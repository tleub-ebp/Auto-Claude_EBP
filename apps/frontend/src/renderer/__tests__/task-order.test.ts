...
function createTestTaskOrder(
	overrides: Partial<TaskOrderState> = {},
): TaskOrderState {
	return {
		backlog: [],
		queue: [],
...
describe("Task Order State Management", () => {
	...
	describe("setTaskOrder", () => {
		...
		it("should preserve all column orders", () => {
			...
			expect(useTaskStore.getState().taskOrder?.human_review).toEqual([
				"task-4",
			]);
	...
	describe("reorderTasksInColumn", () => {
		it("should reorder tasks within a column using arrayMove", () => {
			...
			useTaskStore
				.getState()
				.reorderTasksInColumn("backlog", "task-1", "task-3");
		...
		it("should move task from later position to earlier position", () => {
			...
			useTaskStore
				.getState()
				.reorderTasksInColumn("backlog", "task-4", "task-2");
		...
		it("should handle reordering in different columns", () => {
			...
			useTaskStore
				.getState()
				.reorderTasksInColumn("in_progress", "task-5", "task-3");
		...
		it("should do nothing if taskOrder is null", () => {
			...
			useTaskStore
				.getState()
				.reorderTasksInColumn("backlog", "task-1", "task-2");
		...
		it("should do nothing if activeId is not in the column", () => {
			...
			useTaskStore
				.getState()
				.reorderTasksInColumn("backlog", "nonexistent", "task-2");
		...
		it("should do nothing if overId is not in the column", () => {
			...
			useTaskStore
				.getState()
				.reorderTasksInColumn("backlog", "task-1", "nonexistent");
		...
		it("should do nothing if both activeId and overId are not in the column", () => {
			...
			useTaskStore
				.getState()
				.reorderTasksInColumn("backlog", "nonexistent-1", "nonexistent-2");
		...
		it("should handle reordering with same active and over id (no change)", () => {
			...
			useTaskStore
				.getState()
				.reorderTasksInColumn("backlog", "task-2", "task-2");
	...
	describe("loadTaskOrder", () => {
		...
		it("should use project-specific localStorage keys", () => {
			...
			localStorage.setItem(
				"task-order-state-project-1",
				JSON.stringify(order1),
			);
			localStorage.setItem(
				"task-order-state-project-2",
	...
	describe("saveTaskOrder", () => {
		...
		it("should use project-specific localStorage keys", () => {
			...
			expect(
				localStorage.getItem("task-order-state-my-project-id"),
			).toBeTruthy();
	...
	describe("clearTaskOrder", () => {
		...
		it("should use project-specific localStorage keys", () => {
			localStorage.setItem(
				"task-order-state-project-1",
				JSON.stringify(createTestTaskOrder()),
			);
			localStorage.setItem(
				"task-order-state-project-2",
				JSON.stringify(createTestTaskOrder()),
	...
	describe("moveTaskToColumnTop", () => {
		it("should move task to top of target column", () => {
			...
			useTaskStore
				.getState()
				.moveTaskToColumnTop("task-2", "in_progress", "backlog");
		...
		it("should do nothing if taskOrder is null", () => {
			...
			useTaskStore
				.getState()
				.moveTaskToColumnTop("task-1", "in_progress", "backlog");
	...
	describe("localStorage persistence edge cases", () => {
		...
		it("should isolate different projects completely", () => {
			...
			const orders = {
				...
				"project-c": createTestTaskOrder({
					done: ["c-task-1", "c-task-2", "c-task-3"],
				}),
			};
	...
	describe("order filtering: stale ID removal", () => {
		...
		it("should handle stale IDs across multiple columns", () => {
			...
			useTaskStore.setState({
				tasks,
				taskOrder: orderWithStaleInMultipleColumns,
			});
			...
			// Filter each column
			const validBacklog = taskOrder.backlog.filter((id) =>
				currentTaskIds.has(id),
			);
			const validProgress = taskOrder.in_progress.filter((id) =>
				currentTaskIds.has(id),
			);
	...
	describe("order filtering: new task placement at top", () => {
		...
		it("should addTask to correct column based on task status", () => {
			...
			const newProgressTask = createTestTask({
				id: "new-progress",
				status: "in_progress",
			});
	...
	describe("order filtering: cross-column move updates", () => {
		...
		it("should handle moving from single-item column", () => {
			...
			useTaskStore
				.getState()
				.moveTaskToColumnTop("lone-task", "done", "in_progress");
		...
		it("should handle sequential cross-column moves", () => {
			...
			useTaskStore
				.getState()
				.moveTaskToColumnTop("task-1", "ai_review", "in_progress");
			...
			useTaskStore
				.getState()
				.moveTaskToColumnTop("task-1", "done", "ai_review");
		...
		it("should handle moving task that is already in target column (dedup)", () => {
			...
			const task2Count = updatedOrder?.in_progress.filter(
				(id) => id === "task-2",
			).length;
		...
		it("should preserve unaffected columns during cross-column move", () => {
			...
			useTaskStore
				.getState()
				.moveTaskToColumnTop("backlog-1", "in_progress", "backlog");
	...
	describe("integration: load, reorder, save cycle", () => {
		it("should persist reordering through load/save cycle", () => {
			...
			useTaskStore
				.getState()
				.reorderTasksInColumn("backlog", "task-c", "task-a");
			expect(useTaskStore.getState().taskOrder?.backlog).toEqual([
				"task-c",
				"task-a",
				"task-b",
