...
describe("Task Store", () => {
	...
	describe("updateTask", () => {
		...
		it("should update task by specId", () => {
			...
			useTaskStore
				.getState()
				.updateTask("spec-001", { title: "Updated via specId" });
	...
	describe("updateTaskStatus", () => {
		...
		it("should update updatedAt timestamp", () => {
			...
			expect(
				useTaskStore.getState().tasks[0].updatedAt.getTime(),
		...
		it("should apply reviewReason when provided", () => {
			...
			useTaskStore
				.getState()
				.updateTaskStatus("task-1", "human_review", "plan_review");
		...
		it("should update when only reviewReason changes", () => {
			...
			useTaskStore
				.getState()
				.updateTaskStatus("task-1", "human_review", "completed");
	...
	describe("updateTaskFromPlan", () => {
		it("should extract subtasks from plan", () => {
			...
			expect(useTaskStore.getState().tasks[0].subtasks[0].status).toBe(
				"completed",
			);
		});
		...
		it("should filter by each status type", () => {
			...
			useTaskStore.setState({
				tasks: statuses.map((status) =>
					createTestTask({ id: `task-${status}`, status }),
				),
			});
