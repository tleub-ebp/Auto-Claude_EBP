...
export function useImageUpload({
		...
		async (
			...
			for (const item of itemsToProcess) {
				...
				try {
					...
					onError?.(
						options.isFromPaste
							? errors.processPasteFailed
							: errors.processDropFailed,
					);
				}
			}
	 ...
	 */
	const parseFileReferenceData = useCallback(
		(dataTransfer: DataTransfer): FileReferenceData | null => {
	...
	const handleDrop = useCallback(
		async (e: DragEvent<HTMLElement>) => {
			...
			if (fileRefData) {
				...
				const reference =
					e.dataTransfer.getData("text/plain") || `@${fileRefData.name}`;
