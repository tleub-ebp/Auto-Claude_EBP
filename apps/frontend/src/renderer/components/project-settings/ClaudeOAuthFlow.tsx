...
export function ClaudeOAuthFlow({ onSuccess, onCancel }: ClaudeOAuthFlowProps) {
	...
	const [status, setStatus] = useState<
		"ready" | "authenticating" | "verifying" | "success" | "error"
	>("ready");
	...
	const handleStartAuth = async () => {
		...
		try {
			...
			const result =
				await globalThis.electronAPI.authenticateClaudeProfile(activeProfileId);

			if (!result.success) {
				throw new Error(
					result.error || "Failed to open terminal for authentication",
				);
			}
						...
						<div className="flex items-start gap-4">
							...
							<div className="flex-1">
								...
								<p className="text-sm text-success/80 mt-1">
									{email
										? t("oauth.connectedAs", { email })
										: t("oauth.credentialsSaved")}
								</p>
