...
export class ChangelogService extends EventEmitter {
	...
	getCompletedTasks(
		...
		const specsDir = path.join(
			projectPath,
			specsBaseDir || AUTO_BUILD_PATHS.SPECS_DIR,
		);
	...
	async loadTaskSpecs(
		...
		tasks: Task[],
		specsBaseDir?: string,
	): Promise<TaskSpecContent[]> {
		const specsDir = path.join(
			projectPath,
			specsBaseDir || AUTO_BUILD_PATHS.SPECS_DIR,
		);
		this.debug("loadTaskSpecs called", {
			projectPath,
			specsDir,
		...
		for (const taskId of taskIds) {
			...
			const specDir = path.join(specsDir, task.specId);
			this.debug("Loading spec for task", {
				taskId,
				specId: task.specId,
				specDir,
			});
			...
			try {
				...
				if (existsSync(specPath)) {
					...
					this.debug("Loaded spec.md", {
						specId: task.specId,
						length: content.spec.length,
					});
				}
				...
				if (existsSync(requirementsPath)) {
					content.requirements = JSON.parse(
				...
				if (existsSync(planPath)) {
					content.implementationPlan = JSON.parse(
						readFileSync(planPath, "utf-8"),
					) as ImplementationPlan;
				}
			} catch (error) {
				content.error =
					error instanceof Error ? error.message : "Failed to load spec files";
				this.debug("Error loading spec", {
					specId: task.specId,
					error: content.error,
				});
			}
	...
	getCommits(
		projectPath: string,
		options: import("../../shared/types").GitHistoryOptions,
	): import("../../shared/types").GitCommit[] {
		return getCommits(projectPath, options, this.isDebugEnabled());
	}

	getBranchDiffCommits(
		projectPath: string,
		options: import("../../shared/types").BranchDiffOptions,
	): import("../../shared/types").GitCommit[] {
	...
	suggestVersion(specs: TaskSpecContent[], currentVersion?: string): string {
		...
		for (const spec of specs) {
			...
			if (
				spec.implementationPlan?.workflow_type === "new_feature" ||
	...
	async suggestVersionFromCommits(
		...
		try {
			...
			if (parts.length !== 3 || parts.some(Number.isNaN)) {
				return {
					version: "1.0.0",
					reason: "Invalid current version, resetting to 1.0.0",
				};
			}

			// Use AI to analyze commits and suggest version bump
			const suggester = this.getVersionSuggester();
			const suggestion = await suggester.suggestVersionBump(
				commits,
				currentVersion,
			);
			...
			this.debug(
				"Error in AI version suggestion, falling back to patch bump",
				error,
			);
			// Fallback to patch bump if AI fails
			const [major, minor, patch] = (currentVersion || "1.0.0")
				.split(".")
				.map(Number);
			return {
