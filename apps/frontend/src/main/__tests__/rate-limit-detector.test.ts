...
describe("Rate Limit Detector", () => {
	...
	describe("detectRateLimit", () => {
		...
		it("should NOT false-positive on diagnostic messages mentioning possible causes", async () => {
			...
			const diagnosticOutput =
				"Agent session empty: 0 tool calls, 0b output — possible causes: usage cap reached, auth failure, invalid working directory, or model unavailable.";
	...
	describe("isRateLimitError", () => {
		it("should return true for rate limit errors", async () => {
			...
			expect(isRateLimitError("Limit reached · resets Dec 17 at 6am")).toBe(
				true,
			);
...
describe("Auth Failure Detection", () => {
	...
	describe("detectAuthFailure", () => {
		...
		it('should detect "authentication error" pattern with JSON type', async () => {
			...
			const output =
				'{"type": "authentication_error", "message": "Authentication error occurred"}';
			const result = detectAuthFailure(output);
...
Please authenticate and try again.`;
		...
		it("should handle JSON error responses", async () => {
			...
			const output =
				'{"type":"authentication_error", "error": "unauthorized", "message": "Please authenticate"}';
			const result = detectAuthFailure(output);
		...
		it('should detect "please obtain a new token" pattern with surrounding JSON context', async () => {
			...
			const output =
				'Error: {"error":{"message":"Your session has ended. Please obtain a new token to continue."}}';
			const result = detectAuthFailure(output);
...
describe("Billing Failure Detection", () => {
	...
	describe("detectBillingFailure - spec appendix messages", () => {
		it('should detect "Your credit balance is too low to access the Anthropic API"', async () => {
			...
			const result = detectBillingFailure(
...
Please add credits to continue.`;
		...
		it("should handle JSON error responses with billing failure", async () => {
			...
			const output =
				'{"type":"billing_error","message":"Insufficient credits"}';
			const result = detectBillingFailure(output);
