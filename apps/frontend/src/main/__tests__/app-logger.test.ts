...
describe("Application Logger", () => {
	...
	describe("getRecentLogs", () => {
		...
		it("should respect maxLines parameter", async () => {
			const logContent = Array.from(
				{ length: 10 },
				(_, i) => `[2024-01-15 10:00:0${i}.000] [info] Log line ${i}`,
			).join("\n");
	...
	describe("getRecentErrors", () => {
		...
		it("should respect maxCount parameter", async () => {
			const logContent = Array.from(
				{ length: 50 },
				(_, i) => `[2024-01-15 10:00:0${i}.000] [error] Error ${i}`,
			).join("\n");
	...
	describe("generateDebugReport", () => {
		it("should generate a formatted debug report", async () => {
			const logContent = ["[2024-01-15 10:00:00.000] [error] Test error"].join(
				"\n",
			);
			...
			expect(report).toContain("=== WorkPilot AI Debug Report ===");
	...
	describe("listLogFiles", () => {
		...
		it("should list log files with metadata", async () => {
			...
			writeFileSync(
				path.join(TEST_LOGS_DIR, "main.old.log"),
				"Old log content",
			);
		...
		it("should sort files by modification time (newest first)", async () => {
			...
			if (files.length >= 2) {
				expect(files[0].modified.getTime()).toBeGreaterThanOrEqual(
					files[1].modified.getTime(),
				);
			}
		});
